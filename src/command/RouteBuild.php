...
class RouteBuild extends Command
{
    ...
    {
        $this->setName('routes:build')
            ->addArgument('app', Argument::OPTIONAL, 'Build app routes cache .')
            ->setDescription('Build Annotation routes rule.');
    }
    ...
    protected function buildRoute(string $app = null): string
    {
        ...
        $content = '<?php ' . PHP_EOL . '// 根据注解自动生成的路由规则' . PHP_EOL . 'use sower\\facade\\Route;' . PHP_EOL;

        $layer  = $this->app->config->get('routes.controller_layer');
        $suffix = $this->app->config->get('routes.controller_suffix');
    ...
    protected function getControllerRoute(ReflectionClass $class, string $controller): string
    {
        ...
        if (false !== strpos($comment, '@routes(')) {
            $comment = $this->parseRouteComment($comment);
            $comment = preg_replace('/routes\(\s?([\'\"][\-\_\/\:\<\>\?\$\[\]\w]+[\'\"])\s?\)/is', 'Route::resource(\1,\'' . $controller . '\')', $comment);
     ...
     * @return string
     */
    protected function parseRouteComment(string $comment, string $tag = '@routes('): string
    {
    ...
    protected function getMethodRouteComment(string $controller, ReflectionMethod $reflectMethod, string $group = null)
    {
        ...
        if (false !== strpos($comment, '@routes(')) {
            ...
            if ($suffix = $this->app->routes->config('action_suffix')) {
                $action = substr($action, 0, -strlen($suffix));
            }

            $routes   = $controller . '/' . $action;
            $comment = preg_replace('/routes\s?\(\s?([\'\"][\-\_\/\:\<\>\?\$\[\]\w]+[\'\"])\s?\,?\s?[\'\"]?(\w+?)[\'\"]?\s?\)/is', 'Route::\2(\1,\'' . $routes . '\')' . $group, $comment);
            $comment = preg_replace('/routes\s?\(\s?([\'\"][\-\_\/\:\<\>\?\$\[\]\w]+[\'\"])\s?\)/is', 'Route::rule(\1,\'' . $routes . '\')' . $group, $comment);
