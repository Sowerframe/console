...
class Build extends Command
{
        ...
        foreach ($list as $path => $file) {
            if ('__dir__' == $path) {
                ...
                foreach ($file as $val) {
                    ...
                    switch ($path) {
                        case 'controller': // 控制器
                            if ($this->app->config->get('routes.controller_suffix')) {
                            ...
                            break;
                        case 'model': // 模型
                            $content = "<?php" . PHP_EOL . "namespace {$space};" . PHP_EOL . PHP_EOL . "use sower\Model;" . PHP_EOL . PHP_EOL . "class {$class} extends Model" . PHP_EOL . "{" . PHP_EOL . PHP_EOL . "}";
                            break;
                        case 'view': // 视图
    ...
    {
        $suffix   = $this->app->config->get('routes.controller_suffix') ? 'Controller' : '';
        $filename = $this->basePath . ($appName ? $appName . DIRECTORY_SEPARATOR : '') . 'controller' . DIRECTORY_SEPARATOR . 'Index' . $suffix . '.php';

        if (!is_file($filename)) {
            $content = file_get_contents($this->app->getSowerPath() . 'html' . DIRECTORY_SEPARATOR . 'index.html');
