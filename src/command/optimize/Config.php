...
class Config extends Command
{
    ...
    protected function buildCacheContent(string $app): string
    {
        ...
        $content .= PHP_EOL . substr(php_strip_whitespace($this->app->getSowerPath() . 'helper.php'), 6) . PHP_EOL;
        ...
        $content .= PHP_EOL . '\sowers\Config::set(\sower\App::unserialize(\'' . addslashes(\sower\App::serialize($this->app->config->get())) . '\'));' . PHP_EOL;
        ...
        if (is_file($path . 'event.php')) {
            ...
            if (is_array($event)) {
                if (isset($event['bind'])) {
                    $content .= PHP_EOL . '\sowers\Event::bind(' . (var_export($event['bind'], true)) . ');' . PHP_EOL;
                }

                if (isset($event['listen'])) {
                    $content .= PHP_EOL . '\sowers\Event::listenEvents(' . (var_export($event['listen'], true)) . ');' . PHP_EOL;
                }

                if (isset($event['subscribe'])) {
                    foreach ($event['subscribe'] as $subscribe) {
                        $content .= PHP_EOL . '\sowers\Event::observe(\'' . $subscribe . '\');' . PHP_EOL;
                    }
                }
            }
        }
        ...
        if (is_file($path . 'middleware.php')) {
            ...
            if (is_array($middleware)) {
                $content .= PHP_EOL . '\sower\Container::pull("middleware")->import(' . var_export($middleware, true) . ');' . PHP_EOL;
            }
        }
        ...
        if (is_file($path . 'provider.php')) {
            ...
            if (is_array($provider)) {
                $content .= PHP_EOL . '\sower\Container::getInstance()->bind(' . var_export($provider, true) . ');' . PHP_EOL;
            }
        }
